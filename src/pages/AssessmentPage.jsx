...
export default function AssessmentPage({ currentUser, setActiveTab, assessmentMode = 'evaluation' }) {
  ...
  const fetchIndicators = async () => {
    ...
    try {
      ...
      const url = `http://localhost:3001/api/indicators-by-component/${encodeURIComponent(compId)}?session_id=${sessionId}&major_name=${encodeURIComponent(selectedProgram.majorName)}`;
      ...
      if (!Array.isArray(data) || data.length === 0) {
        const baseRes = await fetch(`http://localhost:3001/api/indicators-by-component/${encodeURIComponent(compId)}`);
        if (baseRes.ok) {
